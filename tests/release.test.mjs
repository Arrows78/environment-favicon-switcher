...
test("updates all version files and creates a strict Keep a Changelog release", () => {
  ...
  assert.equal(JSON.parse(readFileSync(join(root, "package.json"))).version, "2.4.0");
  assert.equal(JSON.parse(readFileSync(join(root, "package-lock.json"))).packages[""].version, "2.4.0");
  assert.equal(JSON.parse(readFileSync(join(root, "manifest.json"))).version, "2.4.0");
...
test("uses and then removes a one-time history baseline", () => {
  ...
  assert.deepEqual(commits.map((commit) => commit.subject), ["ci: add release automation"]);
  ...
  const preparedConfig = JSON.parse(readFileSync(join(fixture, "release.config.json"), "utf8"));
...
test("reports version drift before a release can be merged", () => {
  ...
  assert.ok(
    collectRepositoryErrors(root).some((message) => message.includes("out of sync"))
  );
});
