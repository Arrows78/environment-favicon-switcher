...
test("finds release versions in squash and merge commit messages", () => {
  assert.equal(releaseVersionFromCommitMessage("chore(release): 2.4.0"), "2.4.0");
  assert.equal(
    releaseVersionFromCommitMessage("Merge pull request #12 from automation/release\n\nchore(release): 2.4.0"),
    "2.4.0"
  );
});

test("ignores ordinary commits", () => {
  assert.equal(releaseVersionFromCommitMessage("fix: restore the favicon"), null);
});

test("validates a publish candidate against synchronized metadata", () => {
  const root = createFixture();
  assert.deepEqual(
    inspectPublishCandidate(root, "Merge pull request #12\n\nchore(release): 2.4.0"),
